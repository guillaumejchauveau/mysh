...
/**
 * A structure holding the file descriptor belonging to a pipe, either its read
 * or write end. Used to make a list of all the file descriptors associated with
 * the pipes created by the program.
 */
...
/**
 * The list of all the file descriptors associated with the pipes created by the
 * program.
 */
TAILQ_HEAD(tailhead, pipe_end_descriptor) registered_peds;

/**
 * Initializes the list of pipe ends.
 */
void init_ped_registry() { TAILQ_INIT(&registered_peds); }

/**
 * Adds a pipe end to the list of pipe ends.
 */
...
/**
 * Closes all the file descriptors involved in the pipes created by the program.
 * The list of pipe end descriptors is emptied and the memory freed.
 */
void close_all_pipes() {
  struct pipe_end_descriptor *ped;
  while (TAILQ_FIRST(&registered_peds) != NULL) {
    ped = TAILQ_FIRST(&registered_peds);
...
/**
 * Similar to close_all_pipes() but excludes the file descriptors used by the
 * specified command.
 */
void close_unrelated_pipes(const Command *cmd) {
  struct pipe_end_descriptor *ped;
  struct pipe_end_descriptor *next = TAILQ_FIRST(&registered_peds);
  while (next != NULL) {
    if (next->fd != cmd->fd0 && next->fd != cmd->fd1) {
      ...
      close_file_descriptor(ped->fd);
      free(ped);
      next = TAILQ_FIRST(&registered_peds);
    } else {
      next = TAILQ_NEXT(next, peds);
    }
  }
}
...
/**
 * Frees all memory blocks associated to the specified command.
 */
...
/**
 * Replaces the name of the specified command. The old name is freed from
 * memory.
 */
...
/**
 * Adds an argument to be passed to the specified command when executing.
 */
void add_command_arg(Command *cmd, const char *arg) {
  ...
  cmd->args[args_length - 2] = strdup(arg);
  cmd->args[args_length - 1] = NULL;
}

/**
 * Creates a new pipe and sets file descriptors 0 and 1 for the two provided
 * commands, if they have not been redirected to a file descriptor different
...
/**
 * Sets the file descriptor 0 of the command to the specified file path opened
 * for read.
 */
...
/**
 * Sets the file descriptor 1 of the command to the specified file path opened
 * for write with truncate.
 */
...
/**
 * Executes the specified command in a child process. The command's path is
 * resolved by execvp(2). File descriptors 0 and 1 are replaced by those set in
